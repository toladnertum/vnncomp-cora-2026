...
To run it,
download the CORA repository\footnote{Website: \url{https://cora.in.tum.de}:
If you use git to clone the CORA repository, please also download larger data files using git lfs (large file storage):
i) Install git lfs (\url{https://git-lfs.com/}) and ii) run \texttt{\small git lfs pull} to ensure all data files are downloaded correctly.},
add CORA to the MATLAB path, and type \texttt{installCORA} in the command window.
...
\begin{itemize}
	...
	\item \textbf{Parallel Computing Toolbox} (enables GPU usage)
\end{itemize}
...
\textit{
	Note: If you want to use CORA as part of a repeatability package,
	please visit \cref{sec:repeatability-package} for more information.
}
