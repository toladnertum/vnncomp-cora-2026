...
\begin{equation*}
	\texttt{traj} = \texttt{simulateRandom}(\texttt{sys},\texttt{params},\texttt{options})
\end{equation*}	
...
\begin{tabular}[t]{l p{13cm} }
	...
	$\bullet$~\texttt{options} & struct containing settings for the random simulation \\
		...
		--~\texttt{.points} & number of random initial states (positive integer)\\
	 	--~\texttt{.type} & sampling method: \texttt{'standard'} (default, undefined distribution), \texttt{'gaussian'} (Gaussian distribution), \texttt{'rrt'} (sampling using \textit{rapidly exploring random trees}), \texttt{'falsify'} (extreme trajectories using falsification). 
	 	\end{tabular} \\
		...
		& \begin{tabular}[t]{l p{10cm}}	
	 	...
	 	\end{tabular} \\
	 	& \texttt{.type = 'falsify'}: extreme trajectories via falsification \\
		& \begin{tabular}[t]{l p{10cm}}	
	 	--~\texttt{.dims} & vector containing the dimensions that should be covered by trajectories
	 	\end{tabular}
\end{tabular}
\end{center}
...
\renewcommand{\arraystretch}{1.3}
\begin{tabular}[t]{l p{13cm} }
	$\bullet$~\texttt{traj} & object of class \texttt{trajectory} (see \cref{sec:trajectory}) that stores the simulated trajectories.
\end{tabular}
\end{center}
...
\begin{minipage}[t]{0.58\textwidth}
	\footnotesize
	\input{./MATLABcode/example_simulateRandom}
\end{minipage}
