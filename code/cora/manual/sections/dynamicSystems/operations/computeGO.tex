...
\begin{split}
    ...
    & \texttt{p\_GO} = \texttt{computeGO}(\texttt{sys},\texttt{x0\_ref},\texttt{u\_ref},\texttt{n\_k},\texttt{compute\_params}) 
\end{split}
\end{equation*}
...
\begin{tabular}[t]{l p{12.4cm} }
	...
	$\bullet$~\texttt{compute\_params} & (optional) boolean specifying whether GO parameter should be computed (if \texttt{false}: only compute reference trajectory). 
\end{tabular}
\end{center}
...
\begin{center}
	\begin{minipage}[t]{\textwidth}
		\footnotesize
		\input{./MATLABcode/example_computeGO}
	\end{minipage}
\end{center}
