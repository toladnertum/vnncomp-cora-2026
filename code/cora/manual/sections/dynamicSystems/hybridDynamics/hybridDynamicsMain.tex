...
\begin{itemize}
    ...
    \item a list $\mathbf{T}_i = ( T_1,\dots, T_q)$ of transitions $T_j = ( \mathcal{G}_j,r_j(\cdot),d_j )$, $j = \{1,\dots,q\}$ from the current location to other locations, where $\mathcal{G}_j \subset \Rn$ is a guard set, $r_j\colon \Rn \to \Rn$ is a reset function, and $d_j \in \{1,\dots,p\}$ is the index of the target location.
\end{itemize}
...
\begin{align}
    \begin{split}
        \label{eq:transition}
        T &= \texttt{transition}(\mathcal{G},\texttt{reset},d), \\
        T &= \texttt{transition}(\mathcal{G},\texttt{reset},d,\texttt{label}),
    \end{split}
\end{align}
where
\begin{itemize}
    ...
    \item \texttt{reset} is the reset function $r\colon \mathbb{R}^n \to \mathbb{R}^n$. CORA supports both linear and nonlinear reset functions, defined as
    \begin{align}
        r(x,u) &= A x + B u + c, \qquad A \in \R^{n \times n}, B \in \R^{n \times m}, c \in \mathbb{R}^n, \label{eq:linearReset} \\
        r(x,u) &= h(x,u), \hspace{52pt} h\colon \Rn \to \Rn . \label{eq:nonlinearReset}
    \end{align}
    ...
    For nonlinear reset functions, the \texttt{nonlinearReset(f)} object is initialized with \texttt{f} storing a MATLAB function handle that defines the nonlinear reset function $h(x,u)$.
...
\begin{itemize}
    ...
    \item $\mathbf{T} = (T_1,\dots,T_j)$ is the list of transitions from the current location to other locations represented as an array of \texttt{transition} objects (see \eqref{eq:transition}).
