...
Reset functions can be linear or nonlinear and may also depend on inputs, see \eqref{eq:linearReset}-\eqref{eq:nonlinearReset}.
CORA represents reset functions with the classes \texttt{linearReset} and \texttt{nonlinearReset}.


\paragraph{Guard Sets and Invariant Sets}

The SpaceEx modeling language uses polyhedra for continuous sets.
CORA can store polyhedra with the class \texttt{polytope}.
