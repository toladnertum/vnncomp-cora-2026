...
The \texttt{verify} function implements a branch-and-bound algorithm with the refinement approach described in~\cite{koller2025a}, where the specification is used to iteratively refine the input set until the verification succeeds or a counterexample is found. 
The most important verification options are
\begin{center}
    \renewcommand{\arraystretch}{1.3}
    \begin{longtable}[t]{l p{11cm}}
        --~\texttt{.falsification\_method} & Specify falsification method: {'center','fgsm','zonotack'}. \\
        --~\texttt{.refinement\_method} & Specify input set refinement method: {'naive','zonotack','zonotack-layerwise'}. \\
        --~\texttt{.num\_splits} & Specify the number of pieces a split creates. \\
        --~\texttt{.num\_dimensions} & Specify the number of input dimensions that are simultaneously split. \\
        --~\texttt{.num\_neuron\_splits} & Specify the number of neurons that are simultaneously split. \\
        --~\texttt{.input\_split\_heuristic} & Specify the input splitting heuristic, e.g., {'most-sensitive-input-radius','zono-norm-gradient'}. \\
        --~\texttt{.neuron\_split\_heuristic} & Specify the neuron splitting heuristic, e.g., {'least-unstable','most-sensitive-approx-error','zono-norm-gradient',}.
    \end{longtable}
\end{center}

% at some point, someone might want to move these into a dedicated section
Further examples can be found at \texttt{./cora/examples/nn}.
These also include examples on
explaining neural network predictions~\cite{ladner2025xai},
verifying graph neural networks~\cite{ladner2025gnn},
and computing reduced networks~\cite{ladner2025reduction}.
