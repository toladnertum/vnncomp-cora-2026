...
\subsubsubsection{enlarge}\label{sec:enlarge}

Given a set $\mathcal{S} \subset \Rn$ and a scaling factor $\lambda\in\R_+$, the method \operator{enlarge} is defined as
\begin{equation*}
	\mathcal{S}' = \operator{enlarge}\left(\mathcal{S}, \lambda \right) = \operator{center}(\mathcal{S}) + \lambda (\mathcal{S} - \operator{center}(\mathcal{S})),
\end{equation*}
such that
\begin{align*}
	\mathcal{S} \subseteq \mathcal{S}' &\quad \text{for } \lambda \geq 1, \\
	\mathcal{S} \supseteq \mathcal{S}' &\quad \text{otherwise}.
\end{align*}

Let us demonstrate the method \operator{enlarge} by an example:

\begin{center}
	\begin{minipage}[t]{0.50\textwidth}
		\vspace{10pt}
		\footnotesize
		\input{./MATLABcode/example_enlarge}
	\end{minipage}
	\begin{minipage}[t]{0.3\textwidth}
		\vspace{0pt}
		\centering
		\includetikz{./figures/tikz/set-operations-aux/example_enlarge}
	\end{minipage}
\end{center}
...
\subsubsubsection{reduceUnderApprox}

The method \operator{reduceUnderApprox} computes a reduced-order underapproximation according to \cite{Luetzow2025c} for a given set $\mathcal{S} \subset \Rn$:
\begin{equation}
	\operator{reduceUnderApprox}(\mathcal{S},\texttt{method},\texttt{order}) = \underline{\mathcal{S}}, ~~~ \mathcal{S} \supseteq \underline{\mathcal{S}},
	\label{eq:reduceUnderApprox}
\end{equation}
where the representation size of $\underline{\mathcal{S}}$ is smaller than the one of $\mathcal{S}$. The parameter \texttt{method} in \eqref{eq:reduceUnderApprox} is a string that specifies the algorithm to be applied, see \cref{tab:zono_reductionUnder}. The parameter \texttt{order} in \eqref{eq:reduceUnderApprox} is a measure for the desired representation size of the resulting set $\underline{\mathcal{S}}$. Currently, the method \operator{reduceUnderApprox} is implemented for the set representation \texttt{zonotope} (see \cref{sec:zonotope}). Let us demonstrate the method \operator{reduceUnderApprox} by an example:

\begin{center}
	\begin{minipage}[t]{0.42\textwidth}
		\vspace{10pt}
		\footnotesize
		\input{./MATLABcode/example_reduceUnderApprox}
	\end{minipage}
	\begin{minipage}[t]{0.55\textwidth}
		\vspace{0pt}
		\centering
		\includetikz{./figures/tikz/set-operations-aux/example_reduceUnderApprox}
	\end{minipage}
\end{center}


\begin{table}[h]
	\caption{Underapproximative reduction techniques for zonotopes.}
	\centering
	\label{tab:zono_reductionUnder}
	\begin{tabular}{lll}
		\toprule
		\textbf{Technique} & \textbf{Primary use} & \textbf{Reference} \\
		\midrule
		\texttt{yang} & Reduction by combining closely aligned generators & \cite[Sec.~IV.B]{Yang2022} \\
		\texttt{raghuraman} & Reduction by combining closely aligned generators & \cite[Sec.~5.1]{Raghuraman2022} \\
		\texttt{kochdumper} & Reduction by summing the smallest generators & \cite[Prop. ~.1]{Kochdumper2022a} \\
		\texttt{sadraddini} & Reduction by minimizing the Hausdorff distance & \cite[Corollary.~6 in arXiv version]{Sadraddini2019} \\
		\texttt{cluster} & Reduction by clustering generators & \cite[Sec.~IV.A]{Luetzow2025c} \\
		\texttt{boxCone} & Reduction by computing box underapproximation & \cite[Sec.~IV.B]{Luetzow2025c} \\
		\texttt{scale} & Reduction by scaling template zonotope & \cite[Sec.~IV.C]{Luetzow2025c} \\
		\texttt{nlp} & Reduction by minimizing volume & \cite[Sec.~IV.D]{Luetzow2025c} \\
		\bottomrule
	\end{tabular}
\end{table}
...
\begin{itemize}
	...
	CORA tries to convert automatically between the two if needed (see \textttsmall{readPlotOptions} for details).
\end{itemize}
...
By default, CORA plots sets with edge color and face color with reduced opacity, as visible throughout this section of the manual.
If you prefer to only plot the edge color by default (as was done in previous versions of CORA),
you can change it back to the old behavior using the \textttsmall{CORA\_PLOT\_FILLED} macro.
For individual changes, you can pass \textttsmall{'Filled',true|false} along with the \textttsmall{namevaluepairs}.
To automatically select a color palette for your plots, see \textttsmall{useCORAcolors} for details.
