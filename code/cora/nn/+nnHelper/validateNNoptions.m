...
if isempty(defaultFields)
    defaultFields = {
        ...
        'num_pieces_per_split',2; % number of pieces after split (input and neuron)
        'num_input_dimension_splits',1; % number of dimensions to split into #num_pieces_per_split pieces each
        'num_neuron_splits',0; % number of neurons to split into #num_pieces_per_split pieces each
        'neuron_split_position','zero'; % split offset for neuron splits
        'neuron_split_diversity',0; % diversity weight in [0,1]; >0 reduces the heuristic of neurons whose beta-space split hyperplane aligns with a previous split in the same layer
        ...
        'conzonotope_bound_max_iter',32; % only for 'dual-iter'
        'conzonotope_bound_step_size',1e-1; % only for 'dual-iter'
...
if CHECKS_ENABLED
    ...
    aux_checkFieldClass(options.nn,'num_pieces_per_split', ...
        {'scalar&integer&positive'},structName);
    aux_checkFieldClass(options.nn,'num_input_dimension_splits', ...
        {'scalar&integer&nonnegative'},structName);
    aux_checkFieldClass(options.nn,'num_neuron_splits', ...
        {'scalar&integer&nonnegative'},structName);
    aux_checkFieldStr(options.nn,'neuron_split_position', ...
        {'zero','middle'},structName);
    aux_checkFieldClass(options.nn,'neuron_split_diversity', ...
        {'scalar&nonnegative'},structName);

    % input heuristics
    % Note: Heuristics using approximation error (e.g.,
    % 'most-sensitive-approx-error', 'zono-norm-gradient') or unstable
    % neuron concepts (e.g., 'least-unstable', 'least-unstable-gradient')
    % cannot be used for input splitting or input generators, as these
    % quantities are only available for neuron splitting.
    validInputHeuristics = {...
        'input-radius','most-sensitive-input-radius','zono-norm-gradient'};
    aux_checkFieldStr(options.nn,'input_generator_heuristic', ...
        validInputHeuristics,structName);
